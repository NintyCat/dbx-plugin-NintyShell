...
- **终端工作台**：
  ...
  - 每条命令显示退出码（非零时），输出超过 4 MB 自动截断。
- **实现方式**：Go Sidecar 为每条命令启动 `shell -c` 子进程，通过额外的 fd 3 管道回传退出码与新工作目录（不污染终端输出）；输出以 `shell/output` 事件、命令结束以 `shell/exit` 事件推送。中断通过向进程组发 `SIGINT`（3 秒后升级为 `SIGKILL`）实现。
...
- 不支持 fish（元数据语法不同）；支持 bash / zsh / sh。无 ANSI 颜色渲染（界面会剥离转义序列）。
