...
- **两种连接类型**：
  ...
  - **SSH / SFTP**：主机、端口、用户名，支持密码或私钥（含口令）认证，可从表单直接测试连接；需要 root 权限时可在交互终端手动执行 `su`。
- **终端工作台**（本地与 SSH 通用）：
...
- **权限跟随 SSH 用户**：连接后终端以普通 SSH 用户启动，如需 root 可手动输入 `su`。Docker 面板和 SFTP 文件面板始终使用该 SSH 连接自身的权限，不自动执行 `su` 或 `sudo`；若 Docker 提示 socket 权限不足，请把该 SSH 用户加入 `docker` 组，或改用已有 Docker 权限的账号连接。
- **Docker 应用控制面板**（SSH 连接）：连接时不做 Docker 检测，SSH 会话始终显示 Docker 按钮；点击按钮后才检测并以加载动画读取客户端/守护进程版本、容器与镜像。容器列表只显示状态、镜像、运行状态和端口等摘要；点击 `ⓘ` 后在容器详情内查看完整信息并执行启动、停止、重启、删除和查看最近 200 行日志等操作。镜像详情按钮会展示镜像 ID、标签/摘要、平台、分层、入口/启动命令、环境变量、镜像暴露端口，以及使用该镜像的容器的宿主机端口映射、可访问地址、容器 IP/网关和 Docker 主机网络 IP。删除操作需要二次确认；检测不到 Docker 时在面板内显示“未安装 Docker”页面。
- **设置面板**（标题栏 ⚙）：
...
```bash
CGO_ENABLED=0 dbx-plugin package .
```

`CGO_ENABLED=0` 使 Go 后端构建为纯静态二进制，不依赖系统 glibc——否则在新发行版（glibc ≥ 2.34）上构建、在老系统（如 CentOS 7）运行会报 `libc.so.6: version GLIBC_2.34 not found`。发布 CI 已默认注入该变量。

`dbx-plugin package` 只能在当前平台打包；跨平台打包（如在 Windows 上产出 Linux 包）使用：

```bash
go -C tools/package-cross run . -target linux-x64
```

支持 `linux-x64`、`linux-arm64`、`windows-x64`、`windows-arm64`、`darwin-x64`、`darwin-arm64`，输出到 `dist/`，并在打包前校验 ELF 静态链接（无 `PT_INTERP`/`PT_DYNAMIC`/`GLIBC_` 引用）。
